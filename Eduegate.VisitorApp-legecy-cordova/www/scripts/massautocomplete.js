...
(function () {
    ...
    .directive('massAutocomplete', ['massAutocompleteConfig', '$timeout', '$window', '$document', '$q', function (config, $timeout, $window, $document, $q) {
        return {
            ...
            templateUrl: 'partials/productkeywords.html',
            ...
            controller: ['$scope', function ($scope) {
                    ...
                    if (typeof (term) === 'string' && term.length > 0) {
                        $q.when(current_options.suggest(term),
                          function suggest_succeeded(suggestions) {
                              ...
                              if (suggestions && suggestions.results != undefined && suggestions.results.length > 0) {
                                  // Set unique id to each suggestion so we can
                                  // reference them (aria)
                                  suggestions.results.forEach(function (s) {
                                      if (!s.id) {
                                  ...
                                  $scope.fullresults = { TotalCount:suggestions.TotalCount, results: [].concat(suggestions.results), facets: [].concat(suggestions.facets) };
                                  //$scope.results = [].concat(suggestions.result);
                                  show_autocomplete();
                                  if (current_options.auto_select_first) {
                                      set_selection(1);
                                  }
                              } else {
                                  $scope.fullresults = {TotalCount:0, results: [], facets: [] };
                                  //$scope.results = [];
                                  hide_autocomplete();
                              }
                          },
                ...
                that.detach = function () {
                    ...
                    $scope.selected_index = $scope.fullresults = undefined;
                ...
                function set_selection(i) {
                    ...
                    var selected = $scope.fullresults.results[i];
                ...
                $scope.apply_selection = function (i, j, k,l) {
                    //current_element[0].focus();
                    //if (!$scope.show_autocomplete || i > $scope.fullresults.results.length || i < 0) {
                    //    return;
                    //}
                    ...
                    if (current_options.on_select) {
                        current_options.on_select(selected, j, k,l);
                    }
                };
                    ...
                    bound_events[config.EVENTS.KEYDOWN] = function (e) {
                        ...
                        switch (e.keyCode) {
                            ...
                            case config.KEYS.DOWN:
                                if ($scope.fullresults.results.length > 0) {
                                    if ($scope.show_autocomplete) {
                                        set_selection($scope.selected_index + 1 > $scope.fullresults.results.length - 1 ? 0 : $scope.selected_index + 1);
                                    } else {
                                        show_autocomplete();
                                        set_selection(0);
                                    }
                            ...
                            case config.KEYS.UP:
                                if ($scope.show_autocomplete) {
                                    e.preventDefault();
                                    set_selection($scope.selected_index - 1 >= 0 ? $scope.selected_index - 1 : $scope.fullresults.results.length - 1);
