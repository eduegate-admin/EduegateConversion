app.controller('LayoutController', ['$scope', '$http', '$rootScope', '$state', '$window',
    'rootUrl', '$timeout', 'GetContext', '$q', 'serviceAddToCart', 'serviceCartCount', 'clientSettings', '$compile',  'AuthService',
  'clientSettings',
    function ($scope, $http, $rootScope, $state, $window, rootUrl,
        $timeout, GetContext, $q, serviceAddToCart, serviceCartCount, clientSettings, $compile  , AuthService,
    clientSettings
    ) {
        ...
        $rootScope.IsForceAppUpdate = true;
        $rootScope.updateApp = false;
        ...
        $scope.Init = function () {
            ...
            $rootScope.ShowAllergies = alergiesFlag == "false" ? false : true;
             ...
             if (checkVersionPromise && checkVersionPromise.reject) {
                checkVersionPromise.reject("Aborted");
            }

            if(rootUrl.CurrentAppVersion) {
                checkVersionPromise = $http({
                    url: dataService +
                        "/CheckAppVersion?currentAppVersion=" +
                        (rootUrl.CurrentAppVersion || "1.0.0") +
                        "&appID=" +
                    ...
                    function(result) {
                        ...
                        result = result.data;
                        if (!result.IsUpdated) {
                            $rootScope.updateApp = true;
                            if ($rootScope.IsForceAppUpdate) {
                                $state.go("appupdate", {
                                    loadBlocks: false,
                                });
                            }
        ...
        if (!$rootScope.DefaultCurrency) {
            $rootScope.DefaultCurrency = $rootScope.ClientSettings.DefaultCurrency;
        }

        $timeout(function () {
            if($rootScope.ClientSettings.DefaultDisplayCurrency) {
                $rootScope.DefaultDisplayCurrency = $rootScope.ClientSettings.DefaultDisplayCurrency;
            }
        ...
        $rootScope.goPaymentHistory = function () {
        ...
        $rootScope.getParentPortalUrl = function() {
            context = GetContext.Context();
            $http({
                method: 'GET',
                url: dataService + '/GetParentPortalUrl',
                headers: {
                    "Accept": "application/json;charset=UTF-8",
                    "Content-type": "application/json; charset=utf-8",
                    "CallContext": JSON.stringify(context)
                ...
                $rootScope.ParentPortalUrl = result;
                $rootScope.ShowLoader = false;
        ...
        $scope.SideMenuViewClick = function (shortcut) {
            return $q(function (resolve, reject) {
              $http({
                method: "GET",
                url:
                  ...
                  $rootScope.UserDetails.LoginUserID,
                headers: {
                  Accept: "application/json;charset=UTF-8",
                  "Content-type": "application/json; charset=utf-8",
                  CallContext: JSON.stringify(context),
                ...
                function (result) {
                  ...
                  $scope.StudentID =
                    $scope.SelectedWard != null ? $scope.SelectedWard.data.StudentIID : 0;
                    
                  if (shortcut === 'lesson') {
                    $state.go("lessonplan", { studentID: $scope.StudentID });
                  } else if (shortcut === 'assignment') {
                    $state.go("assignment", { studentID: $scope.StudentID });
                  }
                  resolve();
                },
                function (err) {
