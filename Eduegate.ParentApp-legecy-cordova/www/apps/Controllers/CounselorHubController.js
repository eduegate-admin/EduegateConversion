app.controller("CounselorHubController", ["$scope", "$http", "$state", "rootUrl", "$location", "$rootScope", "$stateParams", "GetContext", "$sce", "$timeout", function ($scope, $http, $state, rootUrl, $location, $rootScope, $stateParams, GetContext, $sce, $timeout) {
...
var url = dataService + "/GetCounselorList";
...
$http({
    ...
    if (!result.IsError && result !== null) {
        $scope.CounselorList = result;
        $rootScope.ShowLoader = false;
    }
}, function (error) {
    // error (optional: handle errors if needed)
}).finally(function () {
    // complete
    $timeout(function () {
