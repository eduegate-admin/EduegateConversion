app.controller("TopicController", ["$scope", "$http", "$state", "rootUrl", "$location", "$rootScope", "$stateParams", "GetContext", "$sce", "$timeout", function ($scope, $http, $state, rootUrl, $location, $rootScope, $stateParams, GetContext, $sce, $timeout) {
  ...
  //To Get Topics
  $scope.FillTopic = function (studentID, subjectID, date) {
    $scope.TopicList = [];
 ...
 // Build base URL
    let url = dataService + '/GetStudentSubjectWiseAgendas?studentID=' + student;

    // Conditionally append subjectID
    if (subjectID) {
        url += '&subjectID=' + subjectID;
    }

    // Conditionally append FilterDate
    if (dateString  ) {
        url += '&date=' + dateString;
    }
    $http({
