...
$scope.GetProducts = function (attachwaypoints) {
    ...
    if (cancelToken) {
        cancelToken.resolve(); // Cancel previous request
    }
    ...
    $http({
        ...
        if (result.WarningMessage && result.PageIndex === 1) {
            $timeout(function () {
                $rootScope.showModalWindow(
                    "notification", "Alert!", result.WarningMessage, "Okay", null, function () {}, null
                );
            }, 500);
        }
        ...
        $timeout(function () {
            ...
            if (result.CatalogGroups && result.CatalogGroups.length > 0) {
                angular.forEach(result.CatalogGroups, function (item) {
                    ...
                    // Optionally set wishlist flags here
                    // $rootScope.SetWishListFlag($scope.ProductListList, index);
                });
                ...
                $scope.attachWayPoints = $scope.isLazyLoadData;
                if ($scope.attachWayPoints) {
                    $scope.InitializeWayPoint();
                }
        ...
        // Optional: handle error (e.g., ignore cancelled requests)
        if (error.status !== -1) {
