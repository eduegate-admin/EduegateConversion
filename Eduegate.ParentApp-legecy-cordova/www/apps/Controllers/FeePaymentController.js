app.controller('FeePaymentController', ['$scope', '$http', '$state', 'rootUrl', '$location', '$rootScope', '$stateParams', 'GetContext', '$sce', '$timeout', function ($scope, $http, $state, rootUrl, $location, $rootScope, $stateParams, GetContext, $sce, $timeout) {
    ...
    $scope.UpdateTypeFee = function (typeRow, studentRow) {
        ...
        if (typeRow.IsPayingNow == false) {
            ...
            angular.forEach(studentRow.StudentFeeDueTypes, function (item) {
                if ((typeRow.FeePeriodID != null && typeRow.IsMandatoryToPay == true && typeRow.IsPayingNow == false) && (item.FeePeriodID == null && item.IsPayingNow == true)) {
                    typeRow.IsPayingNow = true;
                    $rootScope.ShowToastMessage( "Please make sure to pay the mandatory regular fees first!",'error');
                    return false;
                }
            });
            angular.forEach(studentRow.StudentFeeDueTypes, function (item) {
                if ((item.FeePeriodID != null  && item.IsMandatoryToPay == true && item.IsPayingNow == false) && (typeRow.FeePeriodID == null && typeRow.IsPayingNow == true)) {
                    typeRow.IsPayingNow = fasle;
                    $rootScope.ShowToastMessage( "Please make sure to pay the mandatory regular fees first!",'error');
                    return false;
                }
            });
