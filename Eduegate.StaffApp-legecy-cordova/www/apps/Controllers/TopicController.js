app.controller("TopicController", ["$scope", "$http", "$state", "rootUrl", "$location", "$rootScope", "$stateParams", "GetContext", "$sce", "$timeout", function ($scope, $http, $state, rootUrl, $location, $rootScope, $stateParams, GetContext, $sce, $timeout) {
  ...
  $scope.init = function () {
    ...
    $scope.TeacherSubjectList($scope.StudentID);
  ...
  //To Get Topics
$scope.FillTopic = function (subjectID, date) {
  $scope.TopicList = [];
  ...
  let url = dataService + '/GetTeacherSubjectWiseAgendas?';

  url += 'subjectID=' + (subjectID || 0);
  if (dateString) {
    url += '&date=' + dateString;
  }
  ...
  function checkPermissionCallback(status) {
    ...
    $scope.TeacherSubjectList = function (studentID) {
        $scope.AssignmentList = [];

        $http({
            method: 'GET',
            url: dataService + '/Getteachersubjectlist',
            headers: {
                "Accept": "application/json;charset=UTF-8",
                "Content-type": "application/json; charset=utf-8",
                "CallContext": JSON.stringify(context)
            ...
            $scope.teachersubjectlist = result;
