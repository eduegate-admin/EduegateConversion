app.controller("CircularController", ["$scope", "$http", "$state", "rootUrl", "$location", "$rootScope", "$stateParams", "GetContext", "$sce", "$timeout", function ($scope, $http, $state, rootUrl, $location, $rootScope, $stateParams, GetContext, $sce, $timeout) {
  ...
  $scope.FillCirculars = function () {
      $scope.CircularList = [];
        ...
        $http({
            method: "GET",
            url: dataService + "/GetCircularsByEmployee",
            headers: {
            ...
            $timeout(function () {
                // Optionally show/hide loaders or update UI
  ...
  $scope.selectedUserType = ""; // default all

$scope.filterByUserType = function (item) {
    if (!$scope.selectedUserType || $scope.selectedUserType.trim() === "") return true;

    if (!item.CircularUserTypes || !Array.isArray(item.CircularUserTypes)) return false;

    return item.CircularUserTypes.some(function (typeMap) {
        return typeMap.UserType && typeMap.UserType.toLowerCase().includes($scope.selectedUserType.toLowerCase());
    });
};
...
$scope.loadUserTypes = function () {
   $http({
        method: "GET",
        url: dataService + "/GetCircularUserTypes",
        headers: {
            "Accept": "application/json;charset=UTF-8",
            "Content-type": "application/json; charset=utf-8",
            "CallContext": JSON.stringify(context)
        ...
        console.error("Failed to load circular user types:", error);
    }).finally(function () {
        $timeout(function () {
            // Optional post-request logic can go here
        });
    });
